...
# TODO: Refactor indicator variable 

dat_in %>% 
  #filter(value >= 1000) %>%
  filter(location == "Lions") %>% 
  filter(value <= 1000) %>% 
  ...
  geom_line(alpha = 0.5) +
  labs(caption = "Values above 1000 have been removed from this figure") +
...
dat_in_sum_day %>% 
  ...
  facet_wrap(~location, ncol = 4) + #, scales = "free") +
  theme_bw(base_size = 12) 
